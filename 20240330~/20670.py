...
while True:
    try:
        poly1.append((next(now),next(now)))
    except:
        break
now=map(int,input().split())
while True:
    try:
        poly2.append((next(now),next(now)))
    except:
        break
now=map(int,input().split())
while True:
    try:
        points.append((next(now),next(now)))
    except:
        break
ans=[]
res=[]
for point in points:
    if inside(point, poly2): ans.append(point)
    elif inside(point, poly1): res.append(point)
    else: ans.append(point)
